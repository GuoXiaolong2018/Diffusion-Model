...
class ConvBlock(nn.Module):
    def __init__(self,in_channel,out_channel,time_emb_size,qsize,vsize,fsize,cls_emb_size):
        ...
        #时间信息加到每一个像素上，把时间信息揉到每个像素点里 # #8 -> 64
        #过一下线性层，把宽度调成跟通道数一样 #
    ...
    def forward(self,x,t_emb,cls_emb): # t_emb: (batch_size,time_emb_size)
        ...
        t_emb=self.relu(self.time_emb_linear(t_emb)).view(x.size(0),x.size(1),1,1)   # t_emb: (batch_size,out_channel,1,1) #做加法 #
        output=self.seq2(x+t_emb)        # 不改通道数,不改大小
        return self.crossattn(output,cls_emb)   # 图像和引导向量做attention #完成了信息的揉入 #