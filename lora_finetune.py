...
if __name__=='__main__':
    #训练脚本 #
    # 模型
    model=torch.load('model.pt') #已经有的一个预训练模型 #
    ...
    for name,layer in model.named_modules():
        ...
        if any(n in name_cols for n in filter_names) and isinstance(layer,nn.Linear):
            print(name)
            """
            enc_convs.0.crossattn.w_q
            enc_convs.0.crossattn.w_k
            enc_convs.0.crossattn.w_v
            enc_convs.1.crossattn.w_q
            enc_convs.1.crossattn.w_k
            enc_convs.1.crossattn.w_v
            enc_convs.2.crossattn.w_q
            enc_convs.2.crossattn.w_k
            enc_convs.2.crossattn.w_v
            enc_convs.3.crossattn.w_q
            enc_convs.3.crossattn.w_k
            enc_convs.3.crossattn.w_v
            enc_convs.4.crossattn.w_q
            enc_convs.4.crossattn.w_k
            enc_convs.4.crossattn.w_v
            dec_convs.0.crossattn.w_q
            dec_convs.0.crossattn.w_k
            dec_convs.0.crossattn.w_v
            dec_convs.1.crossattn.w_q
            dec_convs.1.crossattn.w_k
            dec_convs.1.crossattn.w_v
            dec_convs.2.crossattn.w_q
            dec_convs.2.crossattn.w_k
            dec_convs.2.crossattn.w_v
            dec_convs.3.crossattn.w_q
            dec_convs.3.crossattn.w_k
            dec_convs.3.crossattn.w_v
            """
            inject_lora(model,name,layer) #name是路径名 
    
    # lora权重的加载
    try:
        restore_lora_state=torch.load('lora.pt')
        model.load_state_dict(restore_lora_state,strict=False)
        print(restore_lora_state.keys())
    except:
        pass 
