...
class UNet(nn.Module):
    def __init__(self,img_channel,channels=[64, 128, 256, 512, 1024],time_emb_size=256,qsize=16,vsize=16,fsize=32,cls_emb_size=32):
        ...
        # 引导词cls转embedding
        self.cls_emb=nn.Embedding(10,cls_emb_size) #可学习 #
        ...
        for i in range(len(channels)-2):
            self.deconvs.append(nn.ConvTranspose2d(channels[-i-1],channels[-i-2],kernel_size=2,stride=2)) #定义，反卷积层 #

        # 每个decoder conv block减少一倍通道数
        self.dec_convs=nn.ModuleList()
        for i in range(len(channels)-2): #定义，每次反卷积之后的操作 #
    ...
    def forward(self,x,t,cls):  # cls是引导词（图片分类ID）
        ...
        # encoder阶段
        residual=[]
        for i,conv in enumerate(self.enc_convs): # 遍历卷积层 #
            x=conv(x,t_emb,cls_emb) #每次下采样，都把时间步信息揉进去 #改变通道数(卷积)->揉进时间信息->卷积 #
            if i!=len(self.enc_convs)-1:
                residual.append(x)
                x=self.maxpools[i](x)

        # decoder阶段
        for i,deconv in enumerate(self.deconvs): #遍历反卷积层 #
            x=deconv(x)
            residual_x=residual.pop(-1)
            x=self.dec_convs[i](torch.cat((residual_x,x),dim=1),t_emb,cls_emb)    # 残差用于纵深channel维 #通道数减半 #
...
if __name__=='__main__':
    ...
    print('batch_noise_t:',batch_noise_t.size()) # torch.Size([2, 1, 48, 48])

    unet=UNet(img_channel=1).to(DEVICE) #输入的图像的通道数是1 #
    batch_predict_noise_t=unet(batch_x_t,batch_t,batch_cls) #模型预测的噪音 #
    print('batch_predict_noise_t:',batch_predict_noise_t.size()) # torch.Size([2, 1, 48, 48])