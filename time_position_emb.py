...
class TimePositionEmbedding(nn.Module):
    def __init__(self,emb_size):
        ...
        self.register_buffer('half_emb',half_emb) #不需要求梯度的参数 #

    def forward(self,t): #输入数字，返回向量 #
        # print("##t.shape:", t.shape) # torch.Size([2])
        t=t.view(t.size(0),1)
        # print("##t.shape:", t.shape) ###t.shape: torch.Size([2, 1])
        # print("##half_emb.shape:", self.half_emb.shape) ###half_emb.shape: torch.Size([4])
        half_emb=self.half_emb.unsqueeze(0).expand(t.size(0),self.half_emb_size)
        # print("##half_emb.shape:", half_emb.shape) # torch.Size([2, 4])
        half_emb_t=half_emb*t
        embs_t = torch.cat((half_emb_t.sin(), half_emb_t.cos()), dim=-1)
        # print("##embs_t.shape:", embs_t.shape) #torch.Size([2, 8])
        return embs_t
    
if __name__=='__main__':
    time_pos_emb=TimePositionEmbedding(8).to(DEVICE)
    t=torch.randint(0,T,(2,)).to(DEVICE)   # 随机2个图片的time时刻
    # print("##t.shape:", t.shape) ###t.shape: torch.Size([2])
    # print(t) #tensor([ 2, 33])
