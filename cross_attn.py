...
class CrossAttention(nn.Module):
    ...
    def forward(self,x,cls_emb): # x:(batch_size,channel,width,height), cls_emb:(batch_size,cls_emb_size) #x：图像, cls_emb：引导词 #
        x=x.permute(0,2,3,1) # x:(batch_size,width,height,channel) #每一个像素点和引导词作交叉注意力 #
        
        # 像素是Query
        Q=self.w_q(x)   # Q: (batch_size,width,height,qsize) #None #
        Q=Q.view(Q.size(0),Q.size(1)*Q.size(2),Q.size(3))   # Q: (batch_size,width*height,qsize) #None #

        # 引导分类是Key和Value
        K=self.w_k(cls_emb) # K: (batch_size,qsize) #None #
        K=K.view(K.size(0),K.size(1),1) # K: (batch_size,qsize,1) #只有一个Key # #None #
        V=self.w_v(cls_emb) # V: (batch_size,vsize) #√ ##>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        V=V.view(V.size(0),1,V.size(1))  # v: (batch_size,1,vsize) #只有一个value ##√ ##>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

        # 注意力打分矩阵Q*K
        attn=torch.matmul(Q,K)/math.sqrt(Q.size(2)) # attn: (batch_size,width*height,1) #None #
        attn=self.softmax(attn) # attn: (batch_size,width*height,1) #None #
        # print(attn) # 就一个Key&value，所以Query对其注意力打分总是1分满分 #None #
