...
class LoraLayer(nn.Module):
    ...
    def forward(self,x):    # x:(batch_size,in_features)
        ...
        lora_output=x@((self.lora_a@self.lora_b)*self.alpha/self.r)    # matmul(x,matmul(lora_a,lora_b)*alpha/r) #搭建一个旁路 #
...
def inject_lora(model,name,layer):
    ...
    cur_layer=model 
    for child in children: #[enc_convs, 0, crossattn]
        cur_layer=getattr(cur_layer,child) # 逐层下探到linear归属的module 
    ...
    setattr(cur_layer,name_cols[-1],lora_layer) # 替换linear #给crossattn添加lora_layer #